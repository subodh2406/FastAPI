...
def update_employee(emp_id: int, updated_employee: Employee):
    ...
    raise HTTPException(status_code=404, detail='Employee Not Found')
# All fields in this model are required. when we are updating the emp 
# If you send only id and name, FastAPI will return a 422 validation error
# because department and age are missing.
# To allow partial data, make fields optional by giving them default values (e.g., = None).
